...
import { ensureAgent } from './agent-update.js'
...
async function main() {
  ...
  console.log(`  Agent version: ${desired.agent?.version || 'none'}`)
  console.log(`  Player version: ${desired.player?.version || 'none'}`)

  // Check for agent self-update FIRST
  // If agent updates, it will restart the service and exit
  if (desired.agent) {
    const agentUpdated = await ensureAgent(
      config.serverUrl,
      desired.agent.version,
      desired.agent.url,
      desired.agent.checksum,
    )
    if (agentUpdated) {
      // Agent will restart shortly, don't continue with other updates
      console.log('Agent updated, waiting for restart...')
      return
    }
  }
