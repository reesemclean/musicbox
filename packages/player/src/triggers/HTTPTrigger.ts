 ...
 * - POST /trigger-update - Trigger agent to check for updates
 *
    ...
    this.server = http.createServer(async (req, res) => {
      ...
      try {
        if (req.method === "POST" && url.pathname === "/scan") {
          ...
          req.method === "POST" &&
          url.pathname === "/trigger-update"
        ) {
          console.log(`🌐 HTTP: /trigger-update - Starting agent update`);
          // Run the agent service to check for and apply updates
          const { exec } = await import("child_process");
          exec("systemctl start musicbox-agent", (error) => {
            if (error) {
              console.error(`Failed to trigger update: ${error.message}`);
            } else {
              console.log(`🔄 Agent update triggered successfully`);
            }
          });
          res.writeHead(200);
          res.end(
            JSON.stringify({ success: true, message: "Update triggered" })
          );
        } else {
          res.writeHead(404);
