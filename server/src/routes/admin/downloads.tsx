...
function DownloadsPage() {
  ...
  const { data: existingSongs } = useSuspenseQuery(songsQueryOptions)

  // Helper to check if song already exists in library
  const isAlreadyInLibrary = (videoId: string) => {
    ...
    const exists = existingSongs.some((song) => song.youtubeVideoId === videoId)
    console.log('Already in library?', exists)
    return exists
  }
            ...
            {searchResults.slice(0, 10).map((song) => (
              ...
              >
                ...
                {isAlreadyInLibrary(song.videoId) ? (
                  <div className="px-4 py-2 bg-slate-600 text-gray-400 font-semibold rounded">
                    Already in Library
                  </div>
                ) : (
                  <button
